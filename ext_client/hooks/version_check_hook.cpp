...
namespace ext_client::hooks::version_check {
  namespace {
    ...
    auto is_version_check_active_process() -> bool {
      ...
      const std::uint32_t vft = ext_client::utils::process_manager::active_child_vftable();
    ...
    auto find_loading_banner_widget(cps_version_check* self) -> cif_static* {
      ...
      if (!self || !gwnd_self) {
        log_msg("[version_check] find_loading_banner_widget: self or gwnd_self is null");
        return nullptr;
      }
      void* child_list = gwnd_self->m_child_list;
      log_msg("[version_check] find_loading_banner_widget: gwnd_self=%p, child_list=%p", gwnd_self, child_list);
      if (!child_list) {
        return nullptr;
      }
      ...
      ext_client::msvc9::child_list_ref::from_sentinel(child_list).for_each([&](std::uint32_t, void* value) {
        ...
        count++;
        std::uint32_t vft = child ? *reinterpret_cast<std::uint32_t*>(child) : 0;
        log_msg("[version_check] child #%d = %p, vftable = 0x%08X", count, child, vft);
      ...
      log_msg("[version_check] find_loading_banner_widget returns found=%p", found);
      return found;
    }
    ...
    auto start_overlay_for_window(HWND hwnd) -> void {
      ...
      loading_splash_overlay::config cfg{};
      cfg.x = rect.left;
      cfg.y = rect.top;
      ...
      if (!loading_splash_overlay::start(hwnd, cfg) && control_panel().log_events) {
        log_msg("[version_check] failed to start loading splash overlay");
      }
    }

    auto restore_window_style() -> void {
      loading_splash_overlay::stop();
    ...
    auto __fastcall set_child_process_detour(void* mgr, void*, int process_type, int activate) -> int {
      ext_client::hooks::title::on_set_child_process(process_type, activate);
      ...
      if (ext_client::hooks::title::control_panel().enabled && ext_client::hooks::title::control_panel().block_process_transition) {
        if (ext_client::hooks::title::control_panel().log_events) {
      ...
      void* child = ext_client::utils::process_manager::active_child();
      if (child == nullptr && result != 0) {
        child = reinterpret_cast<void*>(result);
      }
      ext_client::utils::process_manager::note_set_child_process(child, activate);

      const auto active_vft = ext_client::utils::process_manager::active_child_vftable();
      if (control_panel().log_events) {
        log_msg("[version_check] SetChildProcess(type=%d activate=%d) -> result=0x%08X active_child=%p active_vft=0x%08X",
                process_type,
                activate,
                result,
                ext_client::utils::process_manager::active_child(),
                active_vft);
      }
  ...
  auto install() -> bool {
    ...
    if (!g_hooks.install(g_create_instance, functions::create_instance, &create_instance_detour, "version_check", "create_instance") ||
        ...
        !g_hooks.install(
          g_set_child_process, functions::set_child_process, &set_child_process_detour, "version_check", "set_child_process") ||
