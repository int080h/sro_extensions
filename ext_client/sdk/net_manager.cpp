...
namespace ext_client::net_manager {
  ...
  namespace {
    ...
    auto push_entry(
      packet_direction direction, packet_layer layer, std::uint16_t opcode, std::vector<std::uint8_t> payload, bool blocked, bool modified)
      -> void {
      const auto& control = cfg();
  ...
  auto record_outgoing(cmsg_stream_buffer* msg, bool blocked, bool modified, std::uint16_t opcode) -> void {
    if (!msg) {
      return;
    }
  ...
  auto record_incoming(cmsg_stream_buffer* msg, bool blocked, bool modified, std::uint16_t opcode) -> void {
    if (!msg) {
      return;
    }
  ...
  auto record_cmsg_outgoing(cmsg* pkt, bool blocked, bool modified, std::uint16_t opcode) -> void {
    if (!pkt) {
      return;
    }
    push_entry(
      packet_direction::client_to_server, packet_layer::cmsg, resolve_cmsg_opcode(pkt, opcode), pkt->extract_payload(), blocked, modified);
  }

  auto record_cmsg_incoming(cmsg* pkt, bool blocked, bool modified, std::uint16_t opcode) -> void {
    if (!pkt) {
      return;
    }
    push_entry(
      packet_direction::server_to_client, packet_layer::cmsg, resolve_cmsg_opcode(pkt, opcode), pkt->extract_payload(), blocked, modified);
  }
  ...
  auto register_handlers() -> void {
    ...
    //
    // on_incoming(0xB007, [](packet_view& ctx) -> handler_result {
    //     std::uint8_t result = 0;
    //     ctx.msg->read_u8(result);
    //     return handler_result::pass;
    // });
    //
    // on_outgoing(0x7007, [](packet_view& ctx) -> handler_result {
    //     return handler_result::pass;
    // });
  }
