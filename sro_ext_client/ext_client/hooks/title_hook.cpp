  ...
  namespace {
    ...
    auto __fastcall on_msg_recv_detour(cps_title* self, void*, cmsg_stream_buffer* packet) -> int {
      ...
      if (opcode == ext_client::offsets::cps_title::packets::auth_notify) {
        ...
        using get_client_config_fn = void* (*)();
        auto get_client_config = reinterpret_cast<get_client_config_fn>(ext_client::offsets::cgwnd::functions::get_client_config);
        void* client_config = get_client_config();
        if (client_config) {
          const void* wstr_ptr =
    ...
    auto title_ui_child(cps_title* title, int res_id) -> cgwnd* {
      ...
      return static_cast<cgwnd*>(title->get_ui_child(res_id, true));
    }
    ...
    struct title_child_find_ctx {
      int res_id = -1;
      cgwnd* found = nullptr;
    };

    auto visit_find_title_child_by_id(cgwnd* wnd, void* raw) -> void {
      auto* ctx = static_cast<title_child_find_ctx*>(raw);
      if (!ctx || ctx->found || !cif_manager::is_live_widget(wnd)) {
        return;
      }
      if (cif_manager::unique_id(wnd) == ctx->res_id) {
        ctx->found = wnd;
      }
    }

    auto find_title_child_by_id(cps_title* self, int res_id) -> cgwnd* {
      if (auto* widget = title_ui_child(self, res_id)) {
        if (cif_manager::is_live_widget(widget)) {
          return widget;
        }
      }

      title_child_find_ctx ctx{res_id, nullptr};
      cif_manager::walk_each(reinterpret_cast<cgwnd*>(self), 14, visit_find_title_child_by_id, &ctx);
      return ctx.found;
    }
