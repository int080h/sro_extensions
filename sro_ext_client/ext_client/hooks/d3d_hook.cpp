...
namespace {
  ...
  auto render_frame() -> void {
    ...
    if (imgui_ready && ext_client::menu::is_visible()) {
  ...
  auto CALLBACK wnd_proc(HWND hwnd, UINT msg, WPARAM w_param, LPARAM l_param) -> LRESULT {
    ...
    if (msg == WM_ACTIVATE) {
      if (LOWORD(w_param) == WA_INACTIVE) {
        w_param = MAKEWPARAM(WA_ACTIVE, HIWORD(w_param));
      }
    }

    if (ext_client::menu::is_visible() && ImGui_ImplWin32_WndProcHandler(hwnd, msg, w_param, l_param)) {
      return true;
    }
  ...
  auto __stdcall end_scene_detour(IDirect3DDevice9* device) -> HRESULT {
    ...
    render_frame();
    return g_end_scene.call_original(device);
  }
