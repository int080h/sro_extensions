...
namespace ext_client::hooks::title {
  ...
  namespace {
    ...
    struct find_by_unique_id_ctx {
      int unique_id = -1;
      cgwnd* found = nullptr;
    };

    auto visit_find_by_unique_id(cgwnd* wnd, void* raw) -> void {
      auto* ctx = static_cast<find_by_unique_id_ctx*>(raw);
      if (!ctx || ctx->found || !cif_manager::is_live_widget(wnd)) {
        return;
      }
      if (cif_manager::unique_id(wnd) == ctx->unique_id) {
        ctx->found = wnd;
      }
    }
    ...
    auto title_ui_child(cps_title* title, int res_id) -> cgwnd* {
      ...
      auto* child = static_cast<cgwnd*>(title->get_ui_child(res_id, true));
      if (cif_manager::is_live_widget(child)) {
        return child;
      }

      find_by_unique_id_ctx ctx{res_id, nullptr};
      cif_manager::walk_each(reinterpret_cast<cgwnd*>(title), 14, visit_find_by_unique_id, &ctx);
      return ctx.found;
    }
