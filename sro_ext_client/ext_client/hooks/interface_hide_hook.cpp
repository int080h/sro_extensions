...
namespace ext_client::hooks::interface_hide {
  ...
  auto active_targets() -> calarm_guide_mgr_wnd::promo_target {
    return calarm_guide_mgr_wnd::active_targets();
  }

  auto any_hide_active() -> bool {
    return calarm_guide_mgr_wnd::any_hide_active() || control_panel().hide_survey;
  }
  ...
  auto apply_from_control() -> void {
    g_config_mask = static_cast<unsigned>(calarm_guide_mgr_wnd::active_targets());
  ...
  auto tick() -> void {
    ...
    if (!cg_interface::is_ingame_hud_ready()) {
      ...
      if (iface) {
        if (!g_promo_saw_ingame) {
          ...
          if (++g_promo_enforce_counter >= k_promo_enforce_every_n_ticks) {
            g_promo_enforce_counter = 0;
    ...
    if (++g_survey_enforce_counter >= k_survey_enforce_every_n_ticks) {
      g_survey_enforce_counter = 0;
      apply_survey_hide_now();
    }
  }
