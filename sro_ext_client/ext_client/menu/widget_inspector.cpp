...
namespace ext_client::menu::widget_inspector {
  namespace {
    ...
    auto is_valid_walk_root(cgwnd* root) -> bool {
      if (!root) {
        return false;
      }
      if (!ext_client::msvc9::is_readable_ptr(root, sizeof(void*) * 4)) {
        return false;
      }
      const auto* vftable = *reinterpret_cast<void* const*>(root);
      return ext_client::msvc9::is_readable_ptr(vftable, sizeof(void*) * 8);
    }

    auto root_from_object(void* obj) -> ui_root {
      auto* wnd = cif_manager::browsable_ui_root(obj);
      if (wnd == nullptr) {
        return {nullptr, "unavailable", false};
      }
      return {wnd, cif_manager::ui_type_name(obj), false};
    }

    auto find_root_from_cursor() -> ui_root {
      const cgwnd* cursor = cgwnd::interface_under_cursor();
      for (const cgwnd* walk = cursor; walk != nullptr; walk = walk->parent()) {
        if (auto* wnd = cif_manager::browsable_ui_root(const_cast<cgwnd*>(walk))) {
          return {wnd, cif_manager::ui_type_name(wnd), false};
        }
      }
    ...
    auto resolve_active_process_root() -> ui_root {
      return root_from_object(ext_client::utils::process_manager::resolved_active_child());
    }
    ...
    auto resolve_auto_root() -> ui_root {
      if (cg_interface::is_ingame_hud_ready()) {
        if (auto root = root_from_object(cg_interface::get()); root.wnd != nullptr) {
          return root;
        }
      }

      if (auto root = resolve_active_process_root(); root.wnd != nullptr) {
        return root;
      }

      return find_root_from_cursor();
    }

    auto resolve_ui_root() -> ui_root {
      const auto& s = state();
      if (s.root == root_mode::auto_detect) {
      ...
      switch (s.root) {
        ...
        case root_mode::cg_interface:
          if (cg_interface::is_ready()) {
            return root_from_object(cg_interface::get());
          }
          break;
        case root_mode::ingame_res_map:
          return {nullptr, "IngameResMap@0x1420408", true};
        case root_mode::iface_res_map:
          return {nullptr, "CGInterface+0x374", true};
        default:
          break;
      }
    ...
    auto draw_widget_glow(cgwnd* widget, ImU32 color, float thickness) -> void {
      ...
      // GetBounds() is client-space (same as DevKit DXDrawLine). ImGui on the hooked HWND
      // also uses client coordinates — do not ClientToScreen or the rect shifts when windowed.
    ...
    auto run_search_scan() -> void {
      ...
      for (auto* root_wnd : roots) {
        const auto chunk = cfg.static_only ? cif_manager::walk_static_texts(root_wnd, cfg.max_depth, probe_max_id)
                                           : cif_manager::walk(root_wnd, cfg.max_depth, probe_max_id);
    ...
    auto recurse_widget_tree(cgwnd* elem, recurse_ctx& ctx) -> void;

    auto visit_recurse_child(cgwnd* child, void* user) -> void {
      auto* ctx = static_cast<recurse_ctx*>(user);
      recurse_ctx child_ctx = *ctx;
      ++child_ctx.depth;
      recurse_widget_tree(child, child_ctx);
    }
    ...
    auto recurse_widget_tree(cgwnd* elem, recurse_ctx& ctx) -> void {
      ...
      if (ctx.show_only_visible && !cif_manager::is_visible(elem)) {
        return;
      }
      ...
      if (child_count == 0 || ctx.depth >= ctx.max_depth) {
    ...
    auto draw_res_map_roots_tree(const std::vector<cif_manager::res_map_entry>& entries, recurse_ctx& ctx) -> void {
      if (entries.empty()) {
    ...
    auto draw_toolbar_row() -> void {
      ...
      if (ImGui::SliderInt("depth##wid", &cfg.max_depth, 1, 32)) {
    ...
    auto draw_hover_pick_panel() -> void {
      ...
      cgwnd* target = cgwnd::interface_under_cursor();
      if (!target) {
        ImGui::TextWrapped("Move the mouse over a game widget.");
        return;
      }
    ...
    auto draw_widget_browser() -> void {
      ...
      if (s.root == root_mode::ingame_res_map) {
        draw_res_map_roots_tree(cif_manager::enumerate_ingame_res_map(), ctx);
        return;
      }
      if (s.root == root_mode::iface_res_map) {
        draw_res_map_roots_tree(cif_manager::enumerate_iface_res_map(), ctx);
        return;
      }

      if (!root.wnd) {
        if (s.root == root_mode::active_process) {
          ImGui::TextDisabled("No CProcess child — waits for SetChildProcess or hover a widget and use Auto.");
        } else {
          ImGui::TextDisabled("Root not available — try Auto or hover + Select hovered.");
        }
        return;
      }

      recurse_widget_tree(root.wnd, ctx);
    }

    auto lookup_info_for(cgwnd* widget) -> cif_manager::widget_info {
      for (const auto& item : state().search_results) {
  ...
  auto draw() -> void {
    ...
    const float tree_h = ImGui::GetContentRegionAvail().y - footer_reserve;
    ImGui::BeginChild("widget_tree_host", ImVec2(0.0f, tree_h > 120.0f ? tree_h : 120.0f), ImGuiChildFlags_Border);
    draw_widget_browser();
    ImGui::EndChild();
  ...
  auto draw_overlay() -> void {
    ...
    if (auto* target = cgwnd::interface_under_cursor()) {
      draw_widget_glow(target, IM_COL32(255, 64, 64, 255), 2.0f);
    }
  }
