...
namespace {
  ...
  calarm_guide_mgr_wnd* g_current_instance = nullptr;
...
auto calarm_guide_mgr_wnd::current() -> calarm_guide_mgr_wnd* {
  if (g_current_instance && is_instance(g_current_instance)) {
...
namespace {
  ...
  constexpr unsigned char k_facebook_effect = 2;
  constexpr unsigned char k_magic_lamp_effect = 3;
  constexpr char k_facebook_slot = 6;
  constexpr char k_magic_lamp_slot = 7;

  auto control() -> const ext_client::config::settings::interface_hide_t& {
    return ext_client::config::data().interface_hide;
  }

} // namespace

auto calarm_guide_mgr_wnd::active_targets() -> calarm_guide_mgr_wnd::promo_target {
  using target = calarm_guide_mgr_wnd::promo_target;
  unsigned mask = 0;
  if (control().hide_facebook) {
    mask |= static_cast<unsigned>(target::facebook);
  }
  if (control().hide_magic_lamp) {
    mask |= static_cast<unsigned>(target::magic_lamp);
  }
  if (control().hide_daily_login) {
    mask |= static_cast<unsigned>(target::daily_login);
  }
  if (control().hide_web_item_alarm) {
    mask |= static_cast<unsigned>(target::web_item_alarm);
  }
  if (control().hide_macro_guide) {
    mask |= static_cast<unsigned>(target::macro_guide);
  }
  return static_cast<target>(mask);
}

auto calarm_guide_mgr_wnd::any_hide_active() -> bool {
  const auto& cfg = control();
  return cfg.hide_facebook || cfg.hide_magic_lamp || cfg.hide_daily_login || cfg.hide_web_item_alarm || cfg.hide_macro_guide;
}

auto calarm_guide_mgr_wnd::should_block_guide(const unsigned guide_id) -> bool {
  if (guide_id == static_cast<unsigned>(ext_client::offsets::calarm_guide_mgr_wnd::guide_ids::facebook)) {
    return control().hide_facebook;
  }
  if (guide_id == static_cast<unsigned>(ext_client::offsets::calarm_guide_mgr_wnd::guide_ids::magic_lamp)) {
    return control().hide_magic_lamp;
  }
  if (guide_id == static_cast<unsigned>(ext_client::offsets::calarm_guide_mgr_wnd::guide_ids::daily_login)) {
    return control().hide_daily_login;
  }
  if (guide_id == ext_client::offsets::cg_interface::guide_ids::web_item_alarm) {
    return control().hide_web_item_alarm;
  }
  if (guide_id == ext_client::offsets::calarm_guide_mgr_wnd::guide_ids::macro) {
    return control().hide_macro_guide;
  }
  return false;
}

auto calarm_guide_mgr_wnd::should_block_slot(const char slot_index) -> bool {
  if (slot_index == k_facebook_slot) {
    return control().hide_facebook;
  }
  if (slot_index == k_magic_lamp_slot) {
    return control().hide_magic_lamp;
  }
  return false;
}

auto calarm_guide_mgr_wnd::should_block_effect(const unsigned char fx_index) -> bool {
  if (fx_index == k_facebook_effect) {
    return control().hide_facebook;
  }
  if (fx_index == k_magic_lamp_effect) {
    return control().hide_magic_lamp;
  }
  return false;
}

auto calarm_guide_mgr_wnd::apply_after_create(calarm_guide_mgr_wnd* mgr) -> void {
  if (!mgr || !calarm_guide_mgr_wnd::is_attached_to_iface(mgr)) {
    return;
  }
