...
namespace ext_client::config {
  namespace {
    ...
    auto apply_key(const char* section, const char* key, const char* value) -> void {
      ...
      if (std::strcmp(section, "interface_manager") == 0) {
        auto& mgr = g_settings.interface_manager;
        if (std::strcmp(key, "apply_on_startup") == 0) {
          mgr.apply_on_startup = parse_bool(value, mgr.apply_on_startup);
        } else if (std::strncmp(key, "hide_", 5) == 0) {
          const int index = atoi(key + 5);
          if (index < 0 || index >= ext_client::config::settings::interface_manager_t::max_hidden) {
            return;
          }
          bool ingame = false;
          int res_key = 0;
          char label[64]{};
          if (std::strncmp(value, "ingame:", 7) == 0) {
            ingame = true;
            unsigned parsed = 0;
            if (std::sscanf(value + 7, "%x:%63[^\n]", &parsed, label) >= 1) {
              res_key = static_cast<int>(parsed);
            }
          } else if (std::strncmp(value, "iface:", 6) == 0) {
            unsigned parsed = 0;
            if (std::sscanf(value + 6, "%x:%63[^\n]", &parsed, label) >= 1) {
              res_key = static_cast<int>(parsed);
            }
          }
          if (res_key != 0) {
            if (index >= mgr.hidden_count) {
              mgr.hidden_count = index + 1;
            }
            auto& rule = mgr.hidden[index];
            rule.res_key = res_key;
            rule.ingame_map = ingame;
            if (label[0] != '\0') {
      ...
      if (std::strcmp(section, "graphic") == 0) {
        ...
        if (std::strcmp(key, "d3d_force_hardware_vp") == 0) {
      ...
      if (std::strcmp(section, "target_window") == 0) {
        auto& c = g_settings.target_window;
        if (std::strcmp(key, "enabled") == 0) {
          c.enabled = parse_bool(value, c.enabled);
        } else if (std::strcmp(key, "show_hp_percent") == 0) {
          c.show_hp_percent = parse_bool(value, c.show_hp_percent);
        }
        return;
      }
      ...
      if (std::strcmp(section, "net") == 0 || std::strcmp(section, "packet") == 0 || std::strcmp(section, "net_manager") == 0) {
        auto& n = g_settings.net;
        if (std::strcmp(key, "enabled") == 0) {
          n.enabled = parse_bool(value, n.enabled);
        } else if (std::strcmp(key, "log_events") == 0) {
          n.log_events = parse_bool(value, n.log_events);
        } else if (std::strcmp(key, "pause_capture") == 0) {
          n.pause_capture = parse_bool(value, n.pause_capture);
        } else if (std::strcmp(key, "log_outgoing") == 0) {
          n.log_outgoing = parse_bool(value, n.log_outgoing);
        } else if (std::strcmp(key, "log_incoming") == 0) {
          n.log_incoming = parse_bool(value, n.log_incoming);
        } else if (std::strcmp(key, "capture_cmsg") == 0) {
          n.capture_cmsg = parse_bool(value, n.capture_cmsg);
        } else if (std::strcmp(key, "capture_stream") == 0) {
          n.capture_stream = parse_bool(value, n.capture_stream);
        } else if (std::strcmp(key, "block_outgoing") == 0) {
          n.block_outgoing = parse_bool(value, n.block_outgoing);
        } else if (std::strcmp(key, "block_incoming") == 0) {
          n.block_incoming = parse_bool(value, n.block_incoming);
        } else if (std::strcmp(key, "block_opcode_mode") == 0) {
          n.block_opcode_mode = parse_int(value, n.block_opcode_mode);
        } else if (std::strcmp(key, "block_opcode") == 0) {
          n.block_opcode = static_cast<std::uint16_t>(std::strtoul(value, nullptr, 16));
        } else if (std::strcmp(key, "block_opcode_list") == 0) {
          std::strncpy(n.block_opcode_list, value, sizeof(n.block_opcode_list) - 1);
          n.block_opcode_list[sizeof(n.block_opcode_list) - 1] = '\0';
        } else if (std::strcmp(key, "edit_outgoing") == 0) {
          n.edit_outgoing = parse_bool(value, n.edit_outgoing);
        } else if (std::strcmp(key, "edit_outgoing_opcode") == 0) {
          n.edit_outgoing_opcode = static_cast<std::uint16_t>(std::strtoul(value, nullptr, 16));
        } else if (std::strcmp(key, "edit_outgoing_apply_all") == 0) {
          n.edit_outgoing_apply_all = parse_bool(value, n.edit_outgoing_apply_all);
        } else if (std::strcmp(key, "edit_incoming") == 0) {
          n.edit_incoming = parse_bool(value, n.edit_incoming);
        } else if (std::strcmp(key, "edit_incoming_opcode") == 0) {
          n.edit_incoming_opcode = static_cast<std::uint16_t>(std::strtoul(value, nullptr, 16));
        } else if (std::strcmp(key, "edit_incoming_apply_all") == 0) {
          n.edit_incoming_apply_all = parse_bool(value, n.edit_incoming_apply_all);
        } else if (std::strcmp(key, "log_to_file") == 0) {
          n.log_to_file = parse_bool(value, n.log_to_file);
        } else if (std::strcmp(key, "max_entries") == 0) {
          n.max_entries = parse_int(value, n.max_entries);
        } else if (std::strcmp(key, "file_path") == 0) {
          std::strncpy(n.file_path, value, sizeof(n.file_path) - 1);
          n.file_path[sizeof(n.file_path) - 1] = '\0';
        }
        return;
      }
    }
  ...
  auto save() -> bool {
    ...
    file << "apply_on_startup=" << (s.interface_manager.apply_on_startup ? 1 : 0) << "\n";
    for (int i = 0; i < s.interface_manager.hidden_count; ++i) {
      const auto& rule = s.interface_manager.hidden[i];
      if (rule.res_key == 0) {
        continue;
      }
    ...
    file << "[target_window]\n";
    file << "enabled=" << (s.target_window.enabled ? 1 : 0) << "\n";
    file << "show_hp_percent=" << (s.target_window.show_hp_percent ? 1 : 0) << "\n\n";
    ...
    file << "[net]\n";
    file << "enabled=" << (s.net.enabled ? 1 : 0) << "\n";
    file << "pause_capture=" << (s.net.pause_capture ? 1 : 0) << "\n";
    file << "log_outgoing=" << (s.net.log_outgoing ? 1 : 0) << "\n";
    file << "log_incoming=" << (s.net.log_incoming ? 1 : 0) << "\n";
    file << "capture_cmsg=" << (s.net.capture_cmsg ? 1 : 0) << "\n";
    file << "capture_stream=" << (s.net.capture_stream ? 1 : 0) << "\n";
    file << "block_outgoing=" << (s.net.block_outgoing ? 1 : 0) << "\n";
    file << "block_incoming=" << (s.net.block_incoming ? 1 : 0) << "\n";
    file << "block_opcode_mode=" << s.net.block_opcode_mode << "\n";
    file << "block_opcode=0x" << std::setfill('0') << std::setw(4) << std::hex << std::uppercase << s.net.block_opcode << "\n";
    file << std::dec << std::nouppercase;
    file << "block_opcode_list=" << s.net.block_opcode_list << "\n";
    file << "edit_outgoing=" << (s.net.edit_outgoing ? 1 : 0) << "\n";
    file << "edit_outgoing_opcode=0x" << std::setfill('0') << std::setw(4) << std::hex << std::uppercase << s.net.edit_outgoing_opcode
         << "\n";
    file << std::dec << std::nouppercase;
    file << "edit_outgoing_apply_all=" << (s.net.edit_outgoing_apply_all ? 1 : 0) << "\n";
    file << "edit_incoming=" << (s.net.edit_incoming ? 1 : 0) << "\n";
    file << "edit_incoming_opcode=0x" << std::setfill('0') << std::setw(4) << std::hex << std::uppercase << s.net.edit_incoming_opcode
         << "\n";
    file << std::dec << std::nouppercase;
    file << "edit_incoming_apply_all=" << (s.net.edit_incoming_apply_all ? 1 : 0) << "\n";
    file << "log_events=" << (s.net.log_events ? 1 : 0) << "\n";
    file << "log_to_file=" << (s.net.log_to_file ? 1 : 0) << "\n";
    file << "max_entries=" << s.net.max_entries << "\n";
    file << "file_path=" << s.net.file_path << "\n\n";
  ...
  auto sync_to_runtime() -> void {
    ...
    if (ext_client::config::data().interface_manager.apply_on_startup) {
      ext_client::menu::interface_manager_runtime::apply_saved_hides();
    }
    ext_client::hooks::cps_title_hook::apply_from_control();
    ext_client::render::render_system::get().apply_from_control();
    ext_client::hooks::cps_version_check_hook::apply_from_control();
  }
