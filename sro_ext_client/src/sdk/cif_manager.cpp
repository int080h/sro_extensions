...
namespace {
  ...
  auto is_valid_ui_res_map_at(const void* obj, std::size_t map_byte_offset) -> bool {
    ...
    if (!ext_client::msvc9::is_readable_ptr(map + ext_client::msvc9::res_map_sentinel_offset, sizeof(void*))) {
      return false;
    }
    const auto* sentinel = *reinterpret_cast<void* const*>(map + ext_client::msvc9::res_map_sentinel_offset);
    return ext_client::msvc9::is_game_ptr(sentinel);
  }
  ...
  auto walk_child_list_children(const void* sentinel,
                                ...
                                std::vector<cif_manager::widget_info>& out,
                                std::unordered_set<cgwnd*>& seen) -> void {
    if (!ext_client::msvc9::is_game_ptr(sentinel) || walk_depth_exceeded(depth, max_depth)) {
      return;
    }
  ...
  auto walk_if_wnd_res_map_children(const void* map_obj,
                                    ...
                                    std::vector<cif_manager::widget_info>& out,
                                    std::unordered_set<cgwnd*>& seen) -> void {
    if (!ext_client::msvc9::is_readable_ptr(map_obj, ext_client::msvc9::ui_res_map_size) || walk_depth_exceeded(depth, max_depth)) {
      return;
    }
  ...
  auto walk_each_child_list_children(const void* sentinel,
                                     ...
                                     cif_manager::walk_visitor_fn visit,
                                     void* ctx) -> void {
    if (!ext_client::msvc9::is_game_ptr(sentinel) || walk_depth_exceeded(depth, max_depth) || !visit) {
      return;
    }
  ...
  auto walk_each_if_wnd_res_map_children(const void* map_obj,
                                         ...
                                         cif_manager::walk_visitor_fn visit,
                                         void* ctx) -> void {
    if (!ext_client::msvc9::is_readable_ptr(map_obj, ext_client::msvc9::ui_res_map_size) || walk_depth_exceeded(depth, max_depth) || !visit) {
      return;
    }
  ...
  auto walk_each_gwnd_recursive(
    ...
    if (!ext_client::msvc9::is_game_ptr(root) || walk_depth_exceeded(depth, max_depth) || !visit) {
      return;
    }
...
namespace ext_client::cif_manager {
  ...
  auto enumerate_iface_res_map(int max_entries) -> std::vector<res_map_entry> {
    ...
    return enumerate_res_map(map ? map->raw() : nullptr, max_entries);
  }
  ...
  auto find_title_child(cps_outer_interface* title, int res_id) -> cgwnd* {
    ...
    if (auto* widget = find_ui_child(title, res_id, true)) {
      if (is_live_widget(widget)) {
        return widget;
      }
    }

    if (auto* widget = cgwnd::get_child_by_unique_id(reinterpret_cast<cgwnd*>(title), res_id)) {
      if (is_live_widget(widget)) {
        return widget;
      }
    }
  ...
  auto find_title_child(cgwnd* title_as_gwnd, int res_id) -> cgwnd* {
    return find_title_child(reinterpret_cast<cps_outer_interface*>(title_as_gwnd), res_id);
  }
  ...
  auto for_each_owned_child(cgwnd* parent, child_visitor_fn visit, void* ctx) -> void {
    // Game UI graph (VC2005): intrusive child list + embedded std::map res maps — not ext_client std::vector/map.
    if (!parent || !visit) {
      return;
    }
  ...
  auto client_pointer_pos(int& x, int& y) -> bool {
    ...
    if (ext_client::msvc9::is_readable_ptr(input + ext_client::offsets::input_state::fields::mouse_y + sizeof(int), sizeof(int))) {
      x = ext_client::offsets::field_at<int>(input, ext_client::offsets::input_state::fields::mouse_x);
      y = ext_client::offsets::field_at<int>(input, ext_client::offsets::input_state::fields::mouse_y);
      return true;
    }
  ...
  auto resolve_root_from_hover() -> cgwnd* {
    if (auto* hover = resolve_hovered_widget(false)) {
      if (auto* top = topmost_ui_ancestor(hover)) {
        return top;
      }
    }
    return game_ui_root();
