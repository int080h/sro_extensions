...
namespace ext_client::hooks {
  namespace {
    ...
    auto control_panel() -> cps_title_hook::control& {
      return cps_title_hook::control_panel();
    }
  ...
  auto cps_title_hook::control_panel() -> control& {
    return ext_client::config::data().title;
  }

  static auto on_screen_transition() -> void {
    calarm_guide_mgr_wnd::invalidate_cache();
    sync_active_screens();
  }

  auto cps_title_hook::on_set_child_process(int /*process_type*/, int /*activate*/) -> void {
    on_screen_transition();
  }

  auto cps_title_hook::data_version_label() -> cif_static* {
    return g_data_version_label;
  }

  auto cps_title_hook::exe_version_label() -> cif_static* {
    return g_exe_version_label;
  }

  auto cps_title_hook::captured_version_label_count() -> int {
    return (g_data_version_label ? 1 : 0) + (g_exe_version_label ? 1 : 0);
  }

  auto cps_title_hook::set_version_label_clip_mode(cif_text_clip_mode mode, int ellipsis_width) -> void {
    if (g_data_version_label) {
  ...
  auto cps_title_hook::apply_version_label_layout() -> void {
    apply_version_label_layout_internal();
  }

  auto cps_title_hook::apply_logo_layout() -> void {
    if (auto* title = cps_title::current()) {
      apply_logo_layout_internal(title);
    }
  }

  auto cps_title_hook::apply_version_labels() -> void {
    if (auto* title = cps_title::current()) {
      discover_version_labels(title);
    }
    if (control_panel().override_version_labels) {
  ...
  auto cps_title_hook::apply_from_control() -> void {
  ...
  auto cps_title_hook::uninstall() -> void {
    if (!g_hooks.is_installed()) {
      return;
    }
  ...
  auto cps_title_hook::is_installed() -> bool {
