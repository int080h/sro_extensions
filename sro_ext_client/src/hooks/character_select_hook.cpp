...
namespace ext_client::hooks::character_select {
  namespace {
    ...
    void play_greeting_on_selected_slot(cps_character_select* self) {
      ...
      if (entity) {
        ...
        log_msg("[char_select_hook] play_greeting_on_selected_slot: queued greeting animation for slot %d (entity %p)", slot, entity);
      }
    }

    void update_last_logout_text(cps_character_select* self) {
      log_msg("[char_select_hook] update_last_logout_text called. self = %p", self);
      if (!self)
        return;

      // Get the selected slot index from the global variable (which matches byte_1152638)
      int slot = cps_character_select::selected_slot_index();
      log_msg("[char_select_hook] selected_slot_index = %d", slot);
      if (slot == 255 || slot == -1)
        return;

      // Get the character info structure
      pcinfo_ui* character = self->character_at(slot);
      log_msg("[char_select_hook] character_at(%d) = %p", slot, character);
      if (!character)
        return;
      ...
      if (slot >= 0 && static_cast<std::size_t>(slot) < k_max_char_slots && g_cached_logout_valid[slot]) {
        ...
        log_msg("[char_select_hook] packed_time read from B007 cache = 0x%08X", packed_time);
      } else {
        packed_time = character->packed_time;
        log_msg("[char_select_hook] packed_time fallback read from struct = 0x%08X", packed_time);
      }

      elapsed_logout_parts elapsed{};
      if (!compute_elapsed_logout(packed_time, elapsed)) {
        log_msg("[char_select_hook] Failed to compute elapsed logout for packed_time=0x%08X; using zero defaults", packed_time);
        elapsed = elapsed_logout_parts{};
      }
      log_msg("[char_select_hook] Elapsed parsed: %u month, %u day, %u hour, %u minute",
              elapsed.months,
              elapsed.days,
              elapsed.hours,
              elapsed.minutes);

      // Read the UI format string UIIT_STT_LAST_LOGOUT
      ...
      if (!wstr.empty()) {
        ...
        log_msg("[char_select_hook] Format string from game: %ls", format_str);
      }

      wchar_t buffer[260]{};
      swprintf_s(buffer, 260, format_str, elapsed.months, elapsed.days, elapsed.hours, elapsed.minutes);
      log_msg("[char_select_hook] Formatted buffer: %ls", buffer);

      // Get the child widget with ID 29 (CIFStatic label for Last Logout)
      cgwnd* label =
        reinterpret_cast<cgwnd*>(self->get_ui_child(ext_client::offsets::cps_character_select::ui_ids::last_logout_label, true));
      log_msg("[char_select_hook] label widget (ID 29) = %p", label);
      if (label) {
        ...
        // Set the text of the label using the SDK method
        static_label->set_text(buffer);
        dock_last_logout_label_left(self, static_label);
        log_msg("[char_select_hook] set_text called on label");

        // Make sure it is visible/active (using the cgwnd class method)
        label->set_anim(255, 0.5f, 0.0f, 1);
        log_msg("[char_select_hook] set_anim called on label");
      }

      // Play greeting emote on character selection
        ...
        if (g_greeting_delay_frames > 0) {
          ...
          if (g_greeting_delay_frames == 0) {
            ...
            if (current_entity == g_greeting_target_entity && g_greeting_target_slot == cps_character_select::selected_slot_index()) {
              log_msg("[char_select_hook] on_update_detour: target entity %p is ready, playing greeting", g_greeting_target_entity);
              play_greeting_on_character(g_greeting_target_entity);
            } else {
              log_msg("[char_select_hook] greeting skipped: queued entity changed target=%p current=%p target_slot=%d selected_slot=%d",
                      g_greeting_target_entity,
                      current_entity,
                      g_greeting_target_slot,
                      cps_character_select::selected_slot_index());
            }
    ...
    auto __fastcall on_enter_msg_detour(cps_character_select* self, void*, cprocess_msg* msg) -> int {
      ...
      if (msg) {
        if (msg->msg_id == ext_client::offsets::cps_character_select::wm_user::last_logout) { // wm_user::last_logout
          log_msg("[char_select_hook] on_enter_msg_detour: msg_id = 0x%04X", msg->msg_id);
    ...
    auto __fastcall on_slot_change_detour(cps_character_select* self, void*, int a2, int a3) -> int {
      log_msg("[char_select_hook] on_slot_change_detour: a2 = %d, a3 = %d", a2, a3);
  ...
  auto install() -> bool {
    log_msg("[char_select_hook] install called");
