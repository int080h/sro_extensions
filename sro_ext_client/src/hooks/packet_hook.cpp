  ...
  namespace {
    ...
    auto capture_stream(packet_direction direction, cmsg_stream_buffer* msg, bool blocked, bool modified, const char* capture_point) -> void {
      ...
      if (direction == packet_direction::client_to_server) {
        ext_client::net_manager::record_outgoing(msg, blocked, modified, opcode, capture_point);
      } else {
        ext_client::net_manager::record_incoming(msg, blocked, modified, opcode, capture_point);
      }
    ...
    auto capture_cmsg(packet_direction direction, cmsg* msg, bool blocked, bool modified, const char* capture_point) -> void {
      ...
      if (direction == packet_direction::client_to_server) {
        ext_client::net_manager::record_cmsg_outgoing(msg, blocked, modified, opcode, capture_point);
      } else {
        ext_client::net_manager::record_cmsg_incoming(msg, blocked, modified, opcode, capture_point);
      }
    ...
    // Combat overlay parses B070/B071 from a payload snapshot after vanilla dispatch.
    auto __fastcall stream_dispatch_detour(void* self, void* /*edx*/, cmsg_stream_buffer* msg) -> int {
      std::uint16_t combat_opcode = 0;
      std::vector<std::uint8_t> combat_payload{};

      if (msg && ext_client::config::data().combat_overlay.enabled) {
        const auto opcode = msg->opcode();
        if (opcode == ext_client::combat_packets::opcode_skill_cast_begin || opcode == ext_client::combat_packets::opcode_skill_cast_end) {
          combat_opcode = opcode;
          combat_payload = msg->extract_payload();
        }
      }
      ...
      const int result = g_stream_dispatch.call_original(self, msg);

      if (!combat_payload.empty()) {
        ext_client::combat_packets::handle_payload(combat_opcode, combat_payload);
      }
