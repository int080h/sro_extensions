...
namespace ext_client::config {
  namespace {
    ...
    auto apply_key(const char* section, const char* key, const char* value) -> void {
      ...
      if (std::strcmp(section, "combat_overlay") == 0) {
        auto& c = g_settings.combat_overlay;
        ...
        } else if (std::strcmp(key, "show_dps") == 0) {
          c.show_dps = parse_bool(value, c.show_dps);
        } else if (std::strcmp(key, "show_party_dps") == 0) {
          c.show_party_dps = parse_bool(value, c.show_party_dps);
        } else if (std::strcmp(key, "reset_on_target_death") == 0) {
          c.reset_on_target_death = parse_bool(value, c.reset_on_target_death);
        } else if (std::strcmp(key, "reset_on_target_change") == 0) {
          c.reset_on_target_change = parse_bool(value, c.reset_on_target_change);
        } else if (std::strcmp(key, "combat_timeout_sec") == 0) {
          c.combat_timeout_sec = parse_int(value, c.combat_timeout_sec);
        } else if (std::strcmp(key, "skill_cast_attack_type") == 0) {
          c.skill_cast_attack_type = parse_int(value, c.skill_cast_attack_type);
        } else if (std::strcmp(key, "dps_window_x") == 0) {
          c.dps_window_x = static_cast<float>(atof(value));
        } else if (std::strcmp(key, "dps_window_y") == 0) {
          c.dps_window_y = static_cast<float>(atof(value));
        }
        return;
      }
  ...
  auto save() -> bool {
    ...
    file << "[combat_overlay]\n";
    file << "enabled=" << (s.combat_overlay.enabled ? 1 : 0) << "\n";
    file << "show_hp_percent=" << (s.combat_overlay.show_hp_percent ? 1 : 0) << "\n";
    file << "show_dps=" << (s.combat_overlay.show_dps ? 1 : 0) << "\n";
    file << "show_party_dps=" << (s.combat_overlay.show_party_dps ? 1 : 0) << "\n";
    file << "reset_on_target_death=" << (s.combat_overlay.reset_on_target_death ? 1 : 0) << "\n";
    file << "reset_on_target_change=" << (s.combat_overlay.reset_on_target_change ? 1 : 0) << "\n";
    file << "combat_timeout_sec=" << s.combat_overlay.combat_timeout_sec << "\n";
    file << "skill_cast_attack_type=" << s.combat_overlay.skill_cast_attack_type << "\n";
    file << "dps_window_x=" << s.combat_overlay.dps_window_x << "\n";
    file << "dps_window_y=" << s.combat_overlay.dps_window_y << "\n\n";
  ...
  auto sync_to_runtime() -> void {
    ...
    if (ext_client::config::data().interface_manager.apply_on_startup) {
      ext_client::hooks::interface_manager::apply_saved_hides();
    }
    ext_client::hooks::title::apply_from_control();
    ext_client::hooks::sight_range::apply_settings();
    ext_client::hooks::d3d::apply_from_control();
    ext_client::hooks::version_check::apply_from_control();
  }
